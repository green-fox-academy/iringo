...
int main(void) {
	...
	GPIO_InitTypeDef led_Yellow;
	led_Yellow.Pin = GPIO_PIN_10;
	led_Yellow.Mode = GPIO_MODE_OUTPUT_PP;
	led_Yellow.Pull = GPIO_PULLDOWN;
	led_Yellow.Speed = GPIO_SPEED_HIGH;
	HAL_GPIO_Init(GPIOF, &led_Yellow);
	...
	while (1) {
		int timerValue = __HAL_TIM_GET_COUNTER(&TimHandle); // ugyanaz, mint 	TIM2 -> CNT;
		if (timerValue == 5000) {
